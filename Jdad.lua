...
local PlayerActiveModifyingSpeedToggle = PlayerTab:CreateToggle({
   Name = "Активировать Модификацию скорости",
   CurrentValue = false,
...
local ParagraphNote = GameTab:CreateParagraph({Title = "Внимание", Content = "Киллаура Работает только с топором!"})
...
local weapon = (player.Inventory:FindFirstChild("Старый топор") or player.Inventory:FindFirstChild("Хороший топор") or player.Inventory:FindFirstChild("Сильный топор") or player.Inventory:FindFirstChild("Chainsaw"))
...
local PlayerDistanceAutoChopTreeSlider = GameTab:CreateSlider({
   Name = "(Дистанцию АвтоРубки дерева рекомендуется ставить до 200!)",
   Range = {0, 1000},
   Increment = 0.1,
   Suffix = "Дистанция",
   CurrentValue = 25,
...
for _, bunny in pairs(workspace.Map.Foliage:GetChildren()) do
	if bunny:IsA("Model") and (bunny.Name == "Маленькое дерево" or bunny.Name == "Большое Дерево" or bunny.Name == "TreeBig2")  and bunny.PrimaryPart then
		local distance = (bunny.PrimaryPart.Position - hrp.Position).Magnitude
...
local ActiveEspDistanceToggle = SettingsTab:CreateToggle({
   Name = "Активировать есп на дистанцию",
   CurrentValue = false,
   Flag = "EspDistance",
   Callback = function(Value)
