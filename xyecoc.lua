...
local ParagraphInfoServer = InfoTab:CreateParagraph({Title = "Инфо", Content = "Загрузка"})
...
local ButtonBringAllFuelCanister = BringItemTab:CreateButton({
   Name = "Собрать Топливные Канистры",
   Callback = function(Value)
task.spawn(function()
for _, Obj in pairs(game.workspace.Items:GetChildren()) do
...
local ButtonBringAllSheetMetal = BringItemTab:CreateButton({
   Name = "Собрать Листовой Метал",
   Callback = function(Value)
task.spawn(function()
for _, Obj in pairs(game.workspace.Items:GetChildren()) do
if Obj.Name == "Собрать Семена" and Obj:isA("Model") and Obj.PrimaryPart then 
DragItem(Obj)
end
end
end)
...
local Label = BringItemTab:CreateLabel("Элемент есть на карте:Нет (x"..HowManyItemCanShowUp..")", "rewind")
local TextboxBringNameItem = BringItemTab:CreateInput({
   Name = "Текст",
   CurrentValue = "",
   PlaceholderText = "(Выбери имя, только одно, имя можно узнать через Визуал",
...
Label:Set("Элемент есть на карте:Да "..isInTheMap.." (x"..HowManyItemCanShowUp..")","rewind")
   end,
})
local ButtonBringAllThingsNamedInTextBox = BringItemTab:CreateButton({
   Name = "Бот соберёт все предметы с вашим названием",
   Callback = function(Value)
task.spawn(function()
for _, Obj in pairs(game.workspace.Items:GetChildren()) do
if Obj.Name == TextBoxText and Obj:isA("Model") and Obj.PrimaryPart then 
...
local PlayerActiveModifyingSpeedToggle = PlayerTab:CreateToggle({
   Name = "Включите чтобы модернизировать скорость игрока",
   CurrentValue = false,
...
local PlayerFlySpeedSlider = PlayerTab:CreateSlider({
   Name = "Скорость Полёта (Рекомендуется Ставить от 1 до 5)",
   Range = {0, 10},
   Increment = 0.1,
   Suffix = "Скорость Полёта",
   CurrentValue = 1,
...
Rayfield:Notify({
   Title = "Полёт",
   Content = "Вы не отключите кнопку если нажмёте F",
   Duration = 5,
   Image = "rewind",
})
end
...
local ParagraphNote = GameTab:CreateParagraph({Title = "Внимание", Content = "АвтоРубка Деревьев и КиллАура работают только с топором!"})
local PlayerKillAuraDistanceSlider = GameTab:CreateSlider({
   Name = "Дистанция КиллАуры",
   Range = {25, 10000},
   Increment = 0.1,
   Suffix = "Дистанция",
   CurrentValue = 25,
...
local PlayerDistanceAutoChopTreeSlider = GameTab:CreateSlider({
   Name = "Дистанцию АвтоРубки Деревьев Желательно ставить до 250",
   Range = {0, 500},
   Increment = 0.1,
   Suffix = "Дистанция",
   CurrentValue = 25,
...
local ButtonTeleportToCampfire = PlayerTab:CreateButton({
   Name = "Телепортация К Костру",
   Callback = function(Value)
task.spawn(function()
 game.Players.LocalPlayer.Character:WaitForChild("HumanoidRootPart").CFrame = Workspace.Map.Campground.MainFire.PrimaryPart.CFrame + Vector3.new(0,10,0)
end)
end,
})
local ButtonUnloadCheat = SettingsTab:CreateButton({
   Name = "Облутать Сундуки",
   Callback = function()
  Rayfield:Destroy()
end,
})
local ActiveEspDistanceToggle = SettingsTab:CreateToggle({
   Name = "Активировать Дистанцию Визуала",
   CurrentValue = false,
   Flag = "EspDistance",
   Callback = function(Value)
